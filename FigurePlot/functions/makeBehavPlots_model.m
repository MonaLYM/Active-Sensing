...
%% RT & Switchcount vs. choice
 ...
 figure;
    ...
    bp1 = boxplot(toplot, 'positions', bout.valdiff_abs(usebin_i) - 0.1, 'widths', 0.15, 'colors', [0.72,0.27,1.00], 'symbol', '');
    set(bp1, 'LineWidth', 1.5); 
    hold on;
    plot( bout.valdiff_abs(usebin_i) - 0.1, this_mean(usebin_i), 'r*', 'MarkerSize', 10); hold on;
    scatter(bout.valdiff_abs(usebin_i)-0.1, toplot, 12, 'MarkerFaceColor',[0.97,0.53,0.93], 'MarkerFaceAlpha',0.9,'MarkerEdgeColor', 'none' );
    hold on;
    rate = 0.2;
    for i = 1:length(bout.valdiff_abs)
        ...
        [F, Xi] = ksdensity(tX); 
        x_position = i - 1.1; 
        plot1 = fill(x_position + [0, F, 0] .* rate, [Xi(1), Xi, Xi(end)], [0.72,0.27,1.00], 'EdgeColor', 'none', 'FaceAlpha', 0.4, 'LineWidth', 1.2);
    end
    ...
    hold on;
    bp2 = boxplot(toplot, 'positions', bout.valdiff_abs(usebin_i) + 0.1, 'widths', 0.15, 'colors', [0.22,0.22,0.91], 'symbol', '');
    set(bp2, 'LineWidth', 1.5); 
    hold on;
    plot( bout.valdiff_abs(usebin_i) + 0.1, this_mean(usebin_i), 'r*', 'MarkerSize', 10); hold on;
    scatter(bout.valdiff_abs(usebin_i) + 0.1, toplot, 12, 'MarkerFaceColor',[0.69,0.70,0.98], 'MarkerFaceAlpha',0.9,'MarkerEdgeColor', 'none' );
    hold on;
    for i = 1:length(bout.valdiff_abs)
        tX = toplot(:, i); 
        tX = tX(~isnan(tX)); 
        [F, Xi] = ksdensity(tX); 
        x_position = i - 0.9; 
        plot2 = fill(x_position + [0, F, 0] .* rate, [Xi(1), Xi, Xi(end)], [0.00,0.00,1.00], 'EdgeColor', 'none', 'FaceAlpha', 0.4, 'LineWidth', 1.2);
    end
...
%%
    ...
    subplot(1,4,1); hold on;
    [this_mean,this_se] = getMeanAndSE(toplot);
    errorbar(bout.valdiff_abs(usebin_i), this_mean(usebin_i), this_se(usebin_i), 'Color', [0.72,0.27,1.00], 'LineStyle', 'none', 'LineWidth', 2, 'CapSize', 2, 'Marker','.','MarkerSize', 30);
    xticks(bout.valdiff_abs(usebin_i)); xticklabels({'0', '1', '2'});
