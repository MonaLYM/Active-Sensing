...
rt_choice1_valdiff_abs = nan(length(dstruct),length(valdiffunique_abs));
rt_choice2_valdiff_abs = nan(length(dstruct),length(valdiffunique_abs));
rt_valdiff = nan(length(dstruct),length(valdiffunique));
switchcount_valdiff_abs = nan(length(dstruct),length(valdiffunique_abs));
switchcount_choice1_valdiff_abs = nan(length(dstruct),length(valdiffunique_abs));
switchcount_choice2_valdiff_abs = nan(length(dstruct),length(valdiffunique_abs));
switchcount_valdiff = nan(length(dstruct),length(valdiffunique));
switchcountRT_valdiff_abs = nan(length(dstruct),length(valdiffunique_abs));
switchcountRT_choice1_valdiff_abs = nan(length(dstruct),length(valdiffunique_abs));
switchcountRT_choice2_valdiff_abs = nan(length(dstruct),length(valdiffunique_abs));
switchcountRT_valdiff = nan(length(dstruct),length(valdiffunique));
...
fixdur_mean_valdiff = nan(length(dstruct),length(valdiffunique));
...
for s = 1:length(dstruct)
    ...
    for ti = 1:length(dstruct(s).trialnum)
        ...
        for fix_i = 2:length(dstruct(s).fixitem{ti})-1 %middle
    ...
    fixdur_all_trials = cellfun(@(x) mean(x), dstruct(s).fixdur); % 假设fixdur是一个cell array，每个元素包含一次试验的所有fixdur
    ...
    rt_choice1_mean(s) = mean(dstruct(s).rt(dstruct(s).choice==1));
    ...
    for vi = 1:length(valdiffunique)
        ...
        fixdur_mean_valdiff(s,vi) = mean(fixdur_all_trials(valdiff==valdiffunique(vi)));
        rt_valdiff(s,vi) = mean(dstruct(s).rt(valdiff==valdiffunique(vi)));
        switchcount_valdiff(s,vi) = mean(switchcount(valdiff==valdiffunique(vi)));
        switchcountRT_valdiff(s,vi) = mean(switchcount_rtNorm(valdiff==valdiffunique(vi)));
        % rt_choice1_valdiff(s,vi) = mean(dstruct(s).rt(dstruct(s).choice==1 & valdiff==valdiffunique(vi)));
    end
    for vi = 1:length(valdiffunique_abs)
        ...
        rt_choice1_valdiff_abs(s,vi) = mean(dstruct(s).rt(dstruct(s).choice==1 & abs(valdiff)==valdiffunique_abs(vi)));
        rt_choice2_valdiff_abs(s,vi) = mean(dstruct(s).rt(dstruct(s).choice==2 & abs(valdiff)==valdiffunique_abs(vi)));
        switchcount_valdiff_abs(s,vi) = mean(switchcount(abs(valdiff)==valdiffunique_abs(vi)));
        switchcount_choice1_valdiff_abs(s,vi) = mean(switchcount(dstruct(s).choice==1 & abs(valdiff)==valdiffunique_abs(vi)));
        switchcount_choice2_valdiff_abs(s,vi) = mean(switchcount(dstruct(s).choice==2 & abs(valdiff)==valdiffunique_abs(vi)));
        switchcountRT_valdiff_abs(s,vi) = mean(switchcount_rtNorm(abs(valdiff)==valdiffunique_abs(vi)));
        switchcountRT_choice1_valdiff_abs(s,vi) = mean(switchcount_rtNorm(dstruct(s).choice==1 & abs(valdiff)==valdiffunique_abs(vi)));
        switchcountRT_choice2_valdiff_abs(s,vi) = mean(switchcount_rtNorm(dstruct(s).choice==2 & abs(valdiff)==valdiffunique_abs(vi)));
    end
...
outputstruct.rt_choice1_valdiff_abs = rt_choice1_valdiff_abs;
outputstruct.rt_choice2_valdiff_abs = rt_choice2_valdiff_abs;
outputstruct.rt_valdiff = rt_valdiff;
outputstruct.switchcount_valdiff_abs = switchcount_valdiff_abs;
outputstruct.switchcount_valdiff = switchcount_valdiff;
outputstruct.switchcount_choice1_valdiff_abs = switchcount_choice1_valdiff_abs;
outputstruct.switchcount_choice2_valdiff_abs = switchcount_choice2_valdiff_abs;
outputstruct.switchcountRT_valdiff_abs = switchcountRT_valdiff_abs;
outputstruct.switchcountRT_valdiff = switchcountRT_valdiff;
outputstruct.switchcountRT_choice1_valdiff_abs = switchcountRT_choice1_valdiff_abs;
outputstruct.switchcountRT_choice2_valdiff_abs = switchcountRT_choice2_valdiff_abs;
...
outputstruct.mid_valdiff = fixdur_mean_valdiff;
